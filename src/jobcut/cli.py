...
def _shortcut_content(bin: str, data: str, port: int) -> str:
    """OS-appropriate launcher body. Checks the port first (open browser if already up),
    else launches `jobcut serve --open --port N` with JOBCUT_DATA_DIR set."""
    ...
    # linux (and other posix): a .desktop entry that runs the same curl-then-serve check
    inner = (
...
def cmd_shortcut(args) -> int:
    """Generate an OS-appropriate double-click launcher that opens the console (no terminal)."""
    import os
    from pathlib import Path
    ...
    if args.path:
        ...
        base = desktop if desktop.is_dir() else paths.data_dir()
        dest = base / filename

    dest.parent.mkdir(parents=True, exist_ok=True)
    dest.write_text(_shortcut_content(str(bin), data, args.port))
    if os.name == "posix":
        os.chmod(dest, 0o755)
