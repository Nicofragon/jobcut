...
def _signals(rel) -> dict:
    ...
    Deliberately does NOT surface applicant counts / "competition": that number is only a
    snapshot from the moment we pulled the offer and drifts constantly, so tracking it
    honestly would mean re-scraping every offer daily just for that field — not worth it.
    Posting dates, by contrast, are stable once published.
    """
